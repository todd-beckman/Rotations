...
Mon.prototype.takeDirectDamage = function (move, source, target, points) {
    ...
    if (this.health <= points) {
        points = this.health;
        if (!this.runBeforeEvent("LoseLastHP", move, source, target, points)) {
...
Mon.prototype.takeResidualDamage = function (points, message) {
    ...
    if (!this.runBeforeEvent("TakeResidualDamage")) {
        return;
    }
    game.write(message);
...
Mon.prototype.restore = function (points) {
    ...
    if (!this.runBeforeEvent("Heal", points)) {
        return;
    }
    this.heal(points);
};
...
Mon.prototype.healDrainDamage = function (move, mtype, source, target, points, message) {
    ...
    points = source.runMultiplyEvent("Drain", move, mtype, source, target, points);
    points = target.runMultiplyEvent("FoeDrain", move, mtype, source, target, points);
    points = Math.floor(points);
    if (points == 0) {
        return;
    }
...
*/
Mon.prototype.runBeforeEvent = function (e, move, source, mtype, typeeff) {
    var name = "Before" + e;
